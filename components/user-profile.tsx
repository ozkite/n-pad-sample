  ...
  return (
    <div className="bg-gray-900 rounded-xl p-5 shadow-lg">
      ...
      <div className="flex border-b border-gray-800 mb-4">
        <button
          className={`flex-1 py-2 text-center ${activeTab === "stats" ? "border-b-2 border-[#E53E3E] text-[#E53E3E]" : "text-gray-400"}`}
          onClick={() => setActiveTab("stats")}
        >
          Stats
        </button>
        <button
          className={`flex-1 py-2 text-center ${activeTab === "badges" ? "border-b-2 border-[#E53E3E] text-[#E53E3E]" : "text-gray-400"}`}
          onClick={() => setActiveTab("badges")}
        >
          Badges
        </button>
        <button
          className={`flex-1 py-2 text-center ${activeTab === "saved" ? "border-b-2 border-[#E53E3E] text-[#E53E3E]" : "text-gray-400"}`}
          onClick={() => setActiveTab("saved")}
        >
          Saved
        </button>
        <button
          className={`flex-1 py-2 text-center ${activeTab === "submit" ? "border-b-2 border-[#E53E3E] text-[#E53E3E]" : "text-gray-400"}`}
        ...
        <div>
          ...
          <div className="space-y-3">
            {userBadges.map((badge) => (
              <div
                key={badge.id}
                className={`p-3 rounded-lg flex items-center ${badge.earned ? "bg-[#E53E3E]/10" : "bg-gray-800"}`}
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${badge.earned ? "bg-[#E53E3E] text-white" : "bg-gray-700 text-gray-500"}`}
                >
                  <Award className="w-5 h-5" />
                </div>
                <div className="flex-1">
                  <p className={`font-medium ${badge.earned ? "text-[#E53E3E]" : "text-gray-400"}`}>{badge.name}</p>
                  <p className="text-xs text-gray-400">{badge.description}</p>
                </div>
                {badge.earned && <Badge className="w-5 h-5 text-[#E53E3E]" />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="bg-gray-800 p-4 rounded-lg">
            <div className="flex items-start mb-3">
              <Shield className="w-5 h-5 text-[#E53E3E] mr-2 mt-0.5" />
